    ...
    seq match {
      case node: scala.xml.Node =>
        try {
          ...
          Right(xs map { x => fromXML[A](scala.xml.Elem(node.prefix, node.label, scala.xml.Null, node.scope, false, scala.xml.Text(x)), stack) })
        } catch { case e: Exception => Left(e.toString) }
  ...
  def stringToXML(obj: String, namespace: Option[String], elementLabel: Option[String],
    ...
    elementLabel map { label =>
      scala.xml.Elem(getPrefix(namespace, scope).orNull, label,
        scala.xml.Null,
        scope, false, scala.xml.Text(obj.toString))
    ...
    val rule = new RewriteRule {
      override def transform(n: Node): Seq[Node] = n match {
        case x@Elem(prefix, label, attr, scope, _*) if attr exists(p => p.key == "href") =>
          Elem(prefix, label, attr remove("href"), scope, false, lookupRef((x \ "@href").text.tail): _*)
